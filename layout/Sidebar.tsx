  ...
  return (
    <div className="w-64 bg-slate-100 text-slate-900 border-r border-slate-200 h-screen flex flex-col fixed left-0 top-0 z-50">
      ...
      <nav className="flex-1 p-4 space-y-1">
        <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${activeTab === 'dashboard' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-200'}`}>
          <LayoutDashboard size={18} /> Dashboard
        </button>
        <button onClick={() => setActiveTab('proyectos')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${activeTab === 'proyectos' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-200'}`}>
          <Briefcase size={18} /> Proyectos
        </button>
        <button onClick={() => setActiveTab('tareas')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${activeTab === 'tareas' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-200'}`}>
          <CheckSquare size={18} /> Tareas Diarias
        </button>
        <button onClick={() => setActiveTab('reportes')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition ${activeTab === 'reportes' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-200'}`}>
      ...
      <div className="p-4 border-t border-slate-200 mt-auto">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-cyan-500 flex items-center justify-center text-xs font-bold uppercase shadow-lg border border-slate-600 text-white">
