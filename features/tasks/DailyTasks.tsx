...
interface DailyTasksProps {
  ...
  onAddActivity: (taskId: string, nombre: string) => void;
...
export const DailyTasks: React.FC<DailyTasksProps> = ({ 
  ...
  const handleCreateTask = (e: React.FormEvent) => {
    ...
    onAddTask({
      ...newTaskForm,
      FPlaneadaIniAct: newTaskForm.FPlaneadaIniOri,
      FPlaneadaFinAct: newTaskForm.FPlaneadaFinOri,
    });
    setNewTaskForm({
      ...newTaskForm,
      nombre: '',
  ...
  const selectedProjectForNewTask = proyectos.find(p => p.OT === newTaskForm.OT);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm flex flex-wrap items-center gap-5">
        ...
        <div className="relative flex-1 min-w-[220px]">
          <select value={filterDisciplina} onChange={(e) => setFilterDisciplina(e.target.value)} className="w-full pl-4 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-600 outline-none">
        ...
        <div className="relative flex-1 min-w-[220px]">
          <select value={filterGerente} onChange={(e) => setFilterGerente(e.target.value)} className="w-full pl-4 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold text-slate-600 outline-none">
      ...
      {isAddingTask && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <form onSubmit={handleCreateTask} className="relative bg-white p-10 rounded-[2.5rem] shadow-2xl w-full max-w-2xl animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full p-4 border border-slate-200 rounded-2xl text-sm font-semibold" required value={newTaskForm.ID_Unico_Tarea} onChange={e => setNewTaskForm({...newTaskForm, ID_Unico_Tarea: e.target.value})} placeholder="Ej: T24-001-A" />
                </div>
                <div className="space-y-2">
                  ...
                  <select className="w-full p-4 border border-slate-200 rounded-2xl text-sm font-semibold" required value={newTaskForm.OT} onChange={e => setNewTaskForm({...newTaskForm, OT: e.target.value})}>
                    {proyectos.map(p => (<option key={p.OT} value={p.OT}>{p.OT} - {p.nombre}</option>))}
                  </select>
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[11px] font-black uppercase text-slate-400">Nombre del Entregable</label>
                <input type="text" className="w-full p-4 border border-slate-200 rounded-2xl text-sm font-semibold" required value={newTaskForm.nombre} onChange={e => setNewTaskForm({...newTaskForm, nombre: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-2">
                  <label className="text-[11px] font-black uppercase text-slate-400">Fecha Inicio Ori.</label>
                  <input type="date" className="w-full p-4 border border-slate-200 rounded-2xl text-sm font-semibold" required value={newTaskForm.FPlaneadaIniOri} onChange={e => setNewTaskForm({...newTaskForm, FPlaneadaIniOri: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-black uppercase text-slate-400">Fecha Fin Ori.</label>
                  <input type="date" className="w-full p-4 border border-slate-200 rounded-2xl text-sm font-semibold" required value={newTaskForm.FPlaneadaFinOri} onChange={e => setNewTaskForm({...newTaskForm, FPlaneadaFinOri: e.target.value})} />
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {tareasFiltradas.map(tarea => {
          const taskActivities = actividades.filter(a => a.ID_Tarea === tarea.ID_Unico_Tarea);
          ...
          const isStarted = !!tarea.FRealIni;
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0 ml-1">
                    <h3 className="font-black text-slate-800 text-sm md:text-base truncate">
                      <span className="text-slate-400 font-bold mr-1">[{tarea.OT}]</span> - {tarea.nombre}
                    </h3>
                    <p className="text-[9px] text-slate-400 font-bold uppercase mt-0.5 truncate">
                      {project?.nombre || 'PROYECTO DESCONOCIDO'} • {INITIAL_DISCIPLINAS.find(d => d.id === tarea.id_disciplina)?.nombre}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex flex-col md:grid md:grid-cols-12 divide-x divide-slate-100">
                  ...
                  <div className="grid grid-cols-2 gap-x-10 gap-y-10">
                    <div className="space-y-3">
                      ...
                      <div className="text-[11px] text-slate-500 font-bold space-y-2 bg-slate-50/50 p-4 rounded-3xl">
                        <div className="flex justify-between"><span>Inicio:</span> <span className="text-slate-800">{formatDate(tarea.FPlaneadaIniOri)}</span></div>
                        <div className="flex justify-between"><span>Fin:</span> <span className="text-slate-800">{formatDate(tarea.FPlaneadaFinOri)}</span></div>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Plan Actualizado</span>
                      <div className="space-y-3">
                        <input type="date" className="text-[11px] font-black border-2 border-slate-50 rounded-2xl w-full px-4 py-2.5 outline-none" value={tarea.FPlaneadaIniAct} onChange={e => onUpdateTaskDates(tarea.id, { FPlaneadaIniAct: e.target.value })} />
                        <input type="date" className="text-[11px] font-black border-2 border-slate-50 rounded-2xl w-full px-4 py-2.5 outline-none" value={tarea.FPlaneadaFinAct} onChange={e => onUpdateTaskDates(tarea.id, { FPlaneadaFinAct: e.target.value })} />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-8 md:col-span-7 bg-slate-50/50">
                  ...
                  <div className="flex gap-4 mb-8">
                    ...
                    <button onClick={() => (onAddActivity(tarea.ID_Unico_Tarea, newActivityInput[tarea.ID_Unico_Tarea]), setNewActivityInput({...newActivityInput, [tarea.ID_Unico_Tarea]:''}))} className="bg-blue-600 text-white w-14 h-14 rounded-2xl flex items-center justify-center shadow-xl shadow-blue-200"><Plus /></button>
                  </div>
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-3 custom-scrollbar">
                    {taskActivities.map(a => (
                      <div key={a.id} className="flex items-center justify-between p-5 bg-white border border-slate-100 rounded-[1.75rem]">
                        <span className={`text-sm font-black truncate pr-4 ${a.isCompleted ? 'text-slate-300 line-through' : 'text-slate-700'}`}>{a.nombre}</span>
                        <div className="flex gap-2">
                          <button onClick={() => { if (a.isStarted && a.isCompleted) { onToggleActivity(a.id, 'isStarted'); onToggleActivity(a.id, 'isCompleted'); } else { onToggleActivity(a.id, 'isStarted'); } }} className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase border-2 ${a.isStarted ? 'bg-amber-100 text-amber-700' : 'bg-white text-slate-300'}`}>INICIADA</button>
                          <button onClick={() => { if (!a.isStarted && !a.isCompleted) { window.alert("Debe iniciar la actividad antes de finalizarla"); return; } onToggleActivity(a.id, 'isCompleted'); }} disabled={!a.isStarted && !a.isCompleted} className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase border-2 ${a.isCompleted ? 'bg-green-100 text-green-700' : 'bg-white text-slate-300'}`}>FINALIZADA</button>
                        </div>
                      </div>
                    ))}
                  </div>
